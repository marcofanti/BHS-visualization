...
if (Monitor.prototype = {
        ...
        isIE10: -1 < navigator.userAgent.toString().toLowerCase().indexOf("trident/6"),
        ...
        navigating: !0,
        collectorEndpoint: "http://localhost:8080/BehavioSenseAPI/GetAjaxAsync",
        metaNames: {
            journeyid: "bwjourid",
            userid: "bwuuid",
            sessionid: "bwsessid",
        ...
        createCORSRequest: function(method, url, async) {
            ...
            return "withCredentials" in xhr ? (xhr.open(method, url, async), bw.isIE && !async &&(xhr.timeout = bw.syncTimeout), xhr.setRequestHeader("Content-Type", "text/plain")) : "undefined" != typeof XDomainRequest ? ((xhr = new XDomainRequest).open(method, url, async), async ||(xhr.timeout = bw.syncTimeout)) : xhr = "undefined" != typeof ActiveXObject ? new ActiveXObject("Microsoft.XMLHTTP") : null, null !== xhr && void 0 !== xhr.setRequestHeader && xhr.setRequestHeader("Content-Type", "text/plain"), xhr
        },
        prepHttpStr: function(options) {
            var retStr = "";
            return "" !== options.data.journeyid && void 0 !== options.data.journeyid && (retStr += "journeyid=" + encodeURIComponent(options.data.journeyid)), "" !== options.data.sessionid && void 0 !== options.data.sessionid && (retStr += "&sessionid=" + encodeURIComponent(options.data.sessionid)), "" !== options.data.userid && void 0 !== options.data.userid && (retStr += "&userid=" + encodeURIComponent(options.data.userid)), retStr += "&notes=" + encodeURIComponent(options.data.notes), void 0 !== options.data.behavioData && (retStr += "&data=" + encodeURIComponent(JSON.stringify(options.data.behavioData))), retStr
        },
        ajax: function(options) {
            var ok = !1;
            if (navigator.sendBeacon && (ok = navigator.sendBeacon(bw.collectorEndpoint, bw.prepHttpStr(options))), !ok) {
                var async, url = bw.collectorEndpoint;
                options.type;
                async = options.async, async = !(!bw.isChrome && !bw.isOpera) || !bw.isSafari && (!(bw.isIE9 || bw.isIE10 || bw.isIE11) && !bw.navigating);
                var xhr = bw.createCORSRequest("post", url, async);
                bw.httpStr = bw.prepHttpStr(options), bw.isIOS || bw.isIE11 || bw.isIE10 ? xhr.send(bw.httpStr) : setTimeout(function() {
                    xhr.send(bw.httpStr)
                }, 0)
            }
        },
        ...
        getBehavioData: function() {
            return "object" == typeof this.behavioData && (this.behavioData.push(["w", bw.getDataIntegrity(), bw.getPath()]), this.behavioData)
        },
        submitHandler: function(e) {
            if (bw.hasSent) return !0;
            bw.hasSent = !0, bw.navigating = !1;
            var ajaxData = {
                journeyid: bw.getMeta(bw.metaNames.journeyid),
        ...
        navigateHandler: function() {
            if (!bw.navigating || bw.hasSent) return !0;
            bw.hasSent = !0;
            var ajaxData = {
                journeyid: bw.getMeta(bw.metaNames.journeyid),
                sessionid: bw.getMeta(bw.metaNames.sessionid),
        ...
        checkTarget: function(event, timestamp) {
            var element = document.elementFromPoint(event.clientX, event.clientY),
                data = [];
            null != element && element !== this.lastTarget && void 0 !== element.parentNode && (data[0] = "t", data[1] = element.nodeName + "#" + element.id + "#" + element.parentNode.nodeName + "#" + element.parentNode.id, data[2] = timestamp || bw.getTimestamp(), this.lastTarget = element, bw.addEvent(data))
        },
        ...
        pointerMoveHandler: function(event) {
            if (event.getCoalescedEvents) {
                ...
                for (var base = bw.getTimestamp(), lastTimestamp = event.timeStamp, i = 0; i < events.length; i++) {
                    var e = events[i],
                        ts = base - Math.round(lastTimestamp - (e.timeStamp || lastTimestamp));
        ...
        keyHandler: function(event) {
            if ("undefined" == typeof KeyboardEvent || event instanceof KeyboardEvent) {
                var field, monitorType, data = [],
                    keyCode = 0 == event.keyCode ? 229 : event.keyCode,
                    keyId = keyCode,
                    source = event.currentTarget ? event.currentTarget : event.srcElement,
                    caretPos = 0;
                if (229 != keyCode || "keydown" != event.type || bw.isFirefox || bw.hasFallbackListeners || (bw.fallbackListeners(), bw.hasFallbackListeners = !0), field = source.type + "#" + source.name, null == keyCode && (keyId = keyCode = -500), "a" === (monitorType = bw.getMonitorType(source))) {
                    ...
                    if (229 == keyCode && "keydown" == event.type) bw.setTextLength(field, source.value.length);
                    else if (229 == keyCode && "keyup" == event.type) {
                        ...
                        if (-1 != bw.lastKey || ul < dl) {
                            ...
                            var targetData = bw.getTargetData(field),
                                prevKey = targetData[targetData.length - 1];
        ...
        getMonitorType: function(source) {
            for (i = 0; i < bw.behavioweb_config.anonymous.by_id.length; i++)
                if (bw.behavioweb_config.anonymous.by_id[i] == source.id) return "a";
            for (i = 0; i < bw.behavioweb_config.anonymous.by_name.length; i++)
                if (bw.behavioweb_config.anonymous.by_name[i] == source.name) return "a";
            for (i = 0; i < bw.behavioweb_config.anonymous.by_type.length; i++)
                if (bw.behavioweb_config.anonymous.by_type[i] == source.type) return "a";
            for (i = 0; i < bw.behavioweb_config.masked.by_id.length; i++)
                if (bw.behavioweb_config.masked.by_id[i] == source.id) return "m";
            for (i = 0; i < bw.behavioweb_config.masked.by_name.length; i++)
                if (bw.behavioweb_config.masked.by_name[i] == source.name) return "m";
            for (i = 0; i < bw.behavioweb_config.masked.by_type.length; i++)
                if (bw.behavioweb_config.masked.by_type[i] == source.type) return "m";
            return "n"
        },
        getTargetData: function(target) {
            for (var i = this.behavioData.length - 1; 0 <= i; i--)
                if (this.behavioData[i][1] == target) return this.behavioData[i][2]
        },
            ...
            return [1, v(navigator.serviceWorker), v(navigator.geolocation), v(navigator.cookieEnabled), v(navigator.buildID), v(navigator.productSub), v(navigator.oscpu), v("ontouchstart" in window || 0 < navigator.maxTouchPoints || 0 < navigator.msMaxTouchPoints), c(function() {
                ...
                var map = function(elems, callback, arg) {
                        ...
                        if (void 0 !== length && "number" == typeof length && (0 < length && elems[0] && elems[length - 1] || 0 === length))
                            ...
                            for (key in elems) null != (value = callback(elems[key], key, arg)) && (ret[ret.length] = value);
                ...
                if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
                    if (screen.height)
                        if (screen.height < screen.width) var scrHeight = screen.height,
                            scrWidth = screen.width;
                        else scrHeight = screen.width, scrWidth = screen.height;
                    flmobile = 1
        ...
        startMonitor: function() {
            var i, j, thisForm, fields, field, forms = document.getElementsByTagName("form");
            for (window.onpagehide || null === window.onpagehide && bw.isIOS ? window.addEventListener("pagehide", function(e) {
                    bw.navigateHandler(e)
                }) : void 0 !== window.onbeforeunload ? window.addEventListener ? window.addEventListener("beforeunload", function(e) {
                    bw.navigateHandler(e)
                }) : window.attachEvent && window.attachEvent("onbeforeunload", function(e) {
                    e = e || window.event, bw.navigateHandler(e)
                }) : void 0 !== window.onunload && (window.addEventListener ? window.addEventListener("unload", function(e) {
                    bw.navigateHandler(e)
                }) : window.attachEvent && window.attachEvent("onunload", function(e) {
                    e = e || window.event, bw.navigateHandler(e)
                })), "undefined" != typeof jQuery && jQuery("form").submit(bw.submitHandler), i = 0; i < forms.length; i++) {
            ...
            }, 1e3), -1 !== window.location.pathname.split("?")[0].indexOf(bw.sendOnLoad)) && (ajaxData = {
                journeyid: bw.getMeta(bw.metaNames.journeyid),
                sessionid: bw.getMeta(bw.metaNames.sessionid),
                userid: bw.getMeta(bw.metaNames.userid),
                notes: bw.getMeta(bw.metaNames.notes)
...
"undefined" == typeof console && (this.console = {
    ...
    function() {
        ...
        var h, a, c, i = /[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,
            e = {
                "\b": "\\b",
                "\t": "\\t",
                "\n": "\\n",
        ...
        "function" != typeof JSON.stringify && (JSON.stringify = function(l, j, k) {
            var f;
            if (a = h = "", "number" == typeof k)
                for (f = 0; f < k; f += 1) a += " ";
            else "string" == typeof k && (a = k);
                ...
                switch (p && "object" == typeof p && "function" == typeof p.toJSON && (p = p.toJSON(q)), "function" == typeof c && (p = c.call(n, q, p)), typeof p) {
                    ...
                    case "object":
                        ...
                        if (c && "object" == typeof c)
                            for (f = c.length, l = 0; l < f; l += 1) "string" == typeof c[l] && (r = g(j = c[l], p)) && m.push(b(j) + (h ? ": " : ":") + r);
                        else
                            for (j in p) Object.prototype.hasOwnProperty.call(p, j) && (r = g(j, p)) && m.push(b(j) + (h ? ": " : ":") + r);
                        return r = 0 == m.length ? "{}" : h ? "{\n" + h + m.join(",\n" + h) + "\n" + o + "}" : "{" + m.join(",") + "}", h = o, r
                }
            }("", {
                "": l
