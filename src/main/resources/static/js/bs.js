/*jshint esversion: 6 */
var sent = 0;
var collector = function (window, document, monitorName, config) {
    console.log("Collector inside says hi");
...
console.log("Collector script");

// Initialize collector
collector(window, document, "globbw", {autoload: !0});
/*
let endpoint = NaN;
let tenantId = NaN;
...
window.addEventListener("message", function(event) {
    ...
    if (event.data.type && (event.data.type == "INJECTOR")) {
        console.log("Collector received: " + event.data.url, + " " + event.data.tenantID);
        endpoint=event.data.url;
        tenantId=event.data.tenantID;
    }

    if(event.data.type ){
        const dt =event.data.type;
        switch (dt) {
            case "url":
                console.log("Collector received: " + event.data.url, + " " + event.data.tenantID);
                break;
            case "onBeforeRequest":
                console.log("Collector onBeforeRequest: " + event.data);
                sendData();
                break;
            case "onbeforeunload":
                console.log("Collector onbeforeunload: " + event.data);
                sendData();
                break;
            default:
                console.log("Collector default " + event.data);
...
let userID = 'testIntegration1';
let sessionID = Math.round(Math.random()*100000000);
let ipAddress = "1.1.1.1";

function sendData(username){
	console.log('username = ' + username);
    const endpointUri = "http://openam.example.com:9999/" + "BehavioSenseAPI/GetAjaxAsync";
    console.log("Send Data: " + globbw.getData().length);
    console.log("Send uri: " + endpointUri);
    let tenantId = 'chatdemo';
    let httpData =
        "&sessionid=" + encodeURIComponent(sessionID) +
        "&userid=" + encodeURIComponent(username) +
        "&timing=" + globbw.getData() +
        "&tenantId=" + encodeURIComponent(tenantId) +
        "&userAgent=" + encodeURIComponent("agent") +
        "&ip=" + encodeURIComponent(ipAddress) +
        "&notes=" + encodeURIComponent("testing") +
        "&ip=reportFlags" + encodeURIComponent("0") +
        "&operatorFlags=" + encodeURIComponent("0");

    let ok = navigator.sendBeacon(endpointUri, httpData);
    console.log("Beacon: " + ok)
}

window.onbeforeunload = function(e) {
    // check condition
    console.log("before unload local");
    if (sent == 0) {
        sendData(
